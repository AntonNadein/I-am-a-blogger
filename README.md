...
- ***Настройка фаерволаа***
- Сначала проверьте состояние фаерволаа с помощью команды
```
sudo ufw status
```
- Если фаервола отключен, активируйте его
```
sudo ufw enable
```
...
```
- Проверьте настройки фаервола, чтобы убедиться, что правила применились (порты 80,443 и 22 находятся в состоянии 
ALLOW).
```
sudo ufw status
```
