...
class ServiceIndexTestCase(TestCase):
    def setUp(self):
        ...
        self.user_3 = ModelUser.objects.create_user(username='test_user_3', password='password_3',
                                                    email="test_3@test.ru")
        self.user_4 = ModelUser.objects.create_user(username='test_user_4', password='password_4',
        ...
        self.blog_1 = Blog.objects.create(title='Blog 1', topic=self.topic, blog_text=self.blog_text, view_count=10,
                                          owner=self.user_1)
        self.blog_2 = Blog.objects.create(title='Blog 2', topic=self.topic, blog_text=self.blog_text, view_count=25,
                                          owner=self.user_1)
        self.blog_3 = Blog.objects.create(title='Blog 3', topic=self.topic, blog_text=self.blog_text, view_count=5,
                                          owner=self.user_2)
        self.blog_4 = Blog.objects.create(title='Blog 4', topic=self.topic, blog_text=self.blog_text, view_count=20,
                                          owner=self.user_2)
        self.blog_5 = Blog.objects.create(title='Blog 5', topic=self.topic, blog_text=self.blog_text, view_count=30,
                                          owner=self.user_2)
    ...
    def test_get_archives(self):
        ...
        self.assertEqual(len(archives),
                         2)  # Проверка количества месяцев в архиве (если они все в одном месяце, то будет 1)
