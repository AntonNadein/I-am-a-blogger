...
class ServiceFormTests(TestCase):
    ...
    def test_update_price_new_payment(self):
        ...
        self.service_form.blog = Blog.objects.create(title='Blog 2', topic=self.topic, blog_text=self.blog_text,
                                                     view_count=5, owner=self.user_1)
        self.service_form.update_price()
    ...
    def test_initial_price_no_payment(self):
        ...
        self.service_form.blog_instance = Blog.objects.create(title='Blog 3', topic=self.topic,
                                                              blog_text=self.blog_text, view_count=5,
                                                              owner=self.user_1)
        initial_price = self.service_form.initial_prise()
