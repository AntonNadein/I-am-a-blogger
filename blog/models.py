...
class Blog(models.Model):
    ...
    is_published = models.BooleanField(
        default=True, verbose_name="Опубликовать", help_text="Поставте галочку, что-бы запись видели все"
    )
    view_count = models.PositiveIntegerField(default=0, verbose_name="Количество просмотров")
    like = models.ManyToManyField(ModelUser, blank=True, related_name="blog_subscriber", verbose_name="Лайкнули")
    owner = models.ForeignKey(
        ModelUser, on_delete=models.CASCADE, null=True, blank=True, related_name="blog", verbose_name="Владелец"
    )
    is_paid = models.BooleanField(
        default=False, verbose_name="Платный контент", help_text="Поставте галочку, что-бы добавить оплату"
    )
...
class PaidBlog(models.Model):
    ...
    paid_blog = models.OneToOneField(Blog, on_delete=models.CASCADE, related_name="payment", verbose_name="Блог")
